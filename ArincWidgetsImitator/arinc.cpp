...
{
    dev = openSpiDevice("/dev/spidev32766.0");
...
//             << QCoreApplication::instance()->thread();
    ...
    case static_cast<int>(ArincCodeMessage::grinvichTime):
        if(isLegalWord(message))
        {
            ...
            sec = word & 63;                    //bin 63     =  11 1111
            min = ((word & 4032) >> 6);         //bin 4032   =  1111 1100 0000
            hour = ((word & 126976) >> 12);     //bin 126976 =  1 1111 0000 0000 0000
    ...
    case static_cast<int>(ArincCodeMessage::date):
        if(isLegalData(message))
        {
            ...
            day = ((word & 393216)>>17)*10 + ((word & 122880)>>13);
            month = ((word & 4096)>>12)*10 + (( word & 3840)>>8);
            year = ((word & 240)>>4)*10 + (word & 15);
