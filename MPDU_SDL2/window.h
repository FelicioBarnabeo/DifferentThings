...
public:
    Window();
    explicit Window(std::string title , int x , int y, int w, int h, Uint32 flags, RenderFlags renflags);
    ~Window();
    Window* createWindow(std::string title, int x, int y, int w, int h, Uint32 flags);
    void destroyWindow();
    ...
    void toParrent();
    std::unordered_map<std::string, Page*> pagesBuffer() const;
private:
    Page* m_currentPage;
    SDL_Renderer *m_ren; //This is main render for window, whithout this cant drawing
    SDL_Window *m_window;
    std::unordered_map<std::string, Page *> m_pagesBuffer;
